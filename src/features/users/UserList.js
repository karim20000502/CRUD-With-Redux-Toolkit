...
const UserList = () => {
    ...
    const renderCard = () => users.map(users => (
        <div className="bg-gray-300 p-5 flex items-center justify-between" key={users.id}>
            <div>
                <h2 className="font-bold text-lg text-gray-700">{users.name}</h2>
                <span className="font-normal text-gray-800">{users.email}</span>
            </div>
            <div className="flex gap-4">
                ...
                <button onClick={()=>handleRemoveUser(users.id)}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
